...
## Prepare the repository and input

Resolve this skill's real path through any installation symlink. The repository root is two directories above the skill directory and contains `pyproject.toml`. Use its maintained `local-transcription` CLI; all executable processing code lives there.

If the checkout is missing, use the authenticated GitHub CLI to clone the private `ihoru/local-transcription` repository into the user's project directory, defaulting to `~/projects/my/local-transcription`. Read its README and this skill from that checkout. A missing repository or authentication is a setup blocker, not a reason to invent replacement scripts.

Read `README.md` for environment setup, then use `uv run --locked --project <repo> local-transcription doctor`. Run `uv sync --locked --project <repo>` if dependencies are missing. If models are missing, reuse a known existing model directory with `models install --from-dir`, otherwise run `models install`. Model setup may download public weights; all audio inference stays local. The transcript is reviewed by you, the invoking agent, and enters your context.
...
Speaker detection is on by default and estimates the count. Use `--no-diarization` only when requested. Pass `--speakers N` only when the user supplies the count; do not assume two people. If the user supplies known speaker samples, consult `docs/usage.md` for `--speaker-references`.
...
Read `docs/proofreading.md` before writing edits. Read every part of the TXT, using bounded chunks for long recordings, and use `work/review-source.txt` or `work/transcript.json` to select exact word IDs. Keep track of coverage so reading the opening and ending does not substitute for reviewing the middle.
