...
- In version 0.1.5, native Apple Silicon defaults to `--device metal` using bundled whisper.cpp and large-v3 q5_0. Other platforms default to CPU int8. `--threads` defaults to 8; `--batch-size` (default 4) affects only CPU/CUDA. Install the matching model with `models install --device metal` or `--device cpu`.
